...
static bool initClassReference(ClassObject** pClass, const char* name) {
    ...
    if (name[0] == '[') {
        ...
        result = dvmFindSystemClassNoInit(name); // 重点
    }
...
static bool initClassReferences() {
    ...
    for (i = 0; classes[i].ref != NULL; i++) {
        if (!initClassReference(classes[i].ref, classes[i].name)) { // 重点
            return false;
        }
    }
...
bool dvmFindRequiredClassesAndMembers() {
    ...
    return initClassReferences() // 重点:findClassNoInit
        && initFieldOffsets()
        && initConstructorReferences()
        && initDirectMethodReferences() // 重点:dvmFindDirectMethodByDescriptor
        && initVirtualMethodOffsets() // 重点:dvmFindVirtualMethodByDescriptor
