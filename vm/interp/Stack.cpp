...
static bool dvmPushInterpFrame(Thread* self, const Method* method)
{
    ...
    if (self->interpSave.curFrame != NULL)
        ...
        stackPtr = self->interpStackStart; // 假设是第一个函数调用，则stackPtr指向栈顶
	// 检查栈空间是否够用
    ...
    bool checkAccess)
{
     ...
     */
	// 如果是Jni方法，则调用dvmPushJNIFrame准备栈帧
    if (dvmIsNativeMethod(method)) {
		...
		// 如果是Java方法，则调用dvmPushInterpFrame准备栈帧
 ...
 * self:代表当前执行这个函数的线程
 * method:代表所要执行的函数
 * obj:代表该函数所属的对象，如果为null则为static函数
 * pResult:代表函数的返回值
 * ...:函数参数
 */
    ...
    bool fromJni, JValue* pResult, va_list args)
{
    ...
    ClassObject* clazz;
    u4* ins;
	// 操作线程的stack，准备好栈帧(栈帧的准备是Java方法调用中很关键的一步，需要仔细分析)
    ...
    ins = ((u4*)self->interpSave.curFrame) +
           (method->registersSize - method->insSize); // method->insSize输入参数个数

    //ALOGD("  FP is %p, INs live at >= %p", self->interpSave.curFrame, ins);

    /* put "this" pointer into in0 if appropriate */
	// 如果是非静态函数，则第一个输入参数存储代表this的obj
    ...
    while (*desc != '\0') {
        switch (*(desc++)) {
			...
			// 往输入参数栈空间填入不同类型的参数，ins每次往上挪4个字节
    ...
    if (dvmIsNativeMethod(method)) {
         ...
         */
		/*
		 * Native函数由nativeFunc处理，nativeFunc是
		 * 1.dvmCallJNIMethod:主动注册
		 * 2.dvmResolveNativeMethod:被动注册，而且是第一次调用
		 * 3.如果是虚拟机内部的函数，比如前面两个笑脸的nativeLoad函数
		 */
        (*method->nativeFunc)((u4*)self->interpSave.curFrame, pResult,
                              method, self);
        TRACE_METHOD_EXIT(self, method);
    } else { // Java函数处理逻辑由dvmInterpret完成
