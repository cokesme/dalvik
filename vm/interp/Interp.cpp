...
void dvmInterpret(Thread* self, const Method* method, JValue* pResult)
{
    ...
    self->interpSave.prev = &interpSaveState; // 把旧的interpSave保存起来，就和保存现场一样
    /*
    ...
    self->interpSave.pc = method->insns; // insns:指向该函数对应的指令，待会就是从pc开始，一条指令一条指令进行执行
    ...
    if (gDvm.executionMode == kExecutionModeInterpFast)
        ...
        stdInterp = dvmInterpretPortable; // portable方式对应的指令执行处理

    // Call the interpreter
    (*stdInterp)(self);

    *pResult = self->interpSave.retval; // 存储返回值
