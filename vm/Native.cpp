    ...
    const Method* method, Thread* self)
{
	...
	// dalvik自己有几个native函数，如果是它们，就直接处理
    ...
    void* func = lookupSharedLibMethod(method); // 从虚拟机加载的so库中找
    if (func != NULL) {
        /* found it, point it at the JNI bridge and then call it */
		// 更新Method->nativeFunc和Method->insns，这一块和主动注册一样
        ...
        char** detail)
{
    ...
    *detail = NULL;
     ...
     */
	// 虚拟机对自己加载的so都用一个SharedLib项来表示，判断这个so是不是已经加载过了
