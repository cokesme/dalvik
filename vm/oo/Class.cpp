...
static bool createInitialClasses() {
     ...
     */
	// 创建一个ClassObject对象。前面说了，这个对象是Java中Class信息在VM中的代表。
    ClassObject* clazz = (ClassObject*)
        dvmMalloc(classObjectSize(CLASS_SFIELD_SLOTS), ALLOC_NON_MOVING);
    if (clazz == NULL) {
        return false;
    }
	// 用来设置ClassObject的一些参数
     ...
     */
    bool ok = true;
	// 代表基础数据类型的Class信息都由gDvm对应的成员变量表示
...
bool dvmClassStartup()
{
	...
	// loadedClasses:VM中所有被加载的Class信息都存储在一个Hash表中。
    gDvm.loadedClasses =
     ...
     */
	// classSerialNumber:类序列号。值从0x50000000开始。每加载一个新的class，该值+1
     ...
     */
	// initiatingLoaderList:存储初始类加载器的链表
     ...
     */
	// 创建代表java.lang.Class类的ClassObject以及一些基础数据类型的类信息
	...
	// processClassPath要加载所有的Boot Class，它涉及到类的加载，是重点内容
...
static bool prepareCpe(ClassPathEntry* cpe, bool isBootstrap)
{
    ...
    if ((strcmp(suffix, "jar") == 0) || (strcmp(suffix, "zip") == 0) ||
		...
		// 处理.jar/.zip/.apk结尾的文件。bootclasspath都是.jar的文件
        if (dvmJarFileOpen(cpe->fileName, NULL, &pJarFile, isBootstrap) == 0) {
            cpe->kind = kCpeJar; // 设置ClassPathEntry的kind为KCpeJar
            cpe->ptr = pJarFile; // 设置指针为对应的文件
            return true;
        }
        ...
        if (dvmRawDexFileOpen(cpe->fileName, NULL, &pRawDexFile, isBootstrap) == 0) {
            cpe->kind = kCpeDex; // 设置ClassPathEntry的kind为KCpeDex
            cpe->ptr = pRawDexFile;
            return true;
        }
    } else {
 ...
 */
/*
 * pathStr:为环境变量BOOTCLASSPATH的值，主要是system/framework下的jar包
 * isBootstrap取值为true
 */
static ClassPathEntry* processClassPath(const char* pathStr, bool isBootstrap)
{
	/*
	 * BOOTCLASSPATH包含多个文件，所以这里需要把它们拆成一个一个的ClassPathEntry，
	 * 每一个ClassPathEntry其实都指向一个Jar包，一直到prepareCpe()
	 */
...
static int hashcmpClassByCrit(const void* vclazz, const void* vcrit)
{
	 ...
	 * 两个类是否相同的比较条件：
	 * 1.类名一样
	 * 2.classloader一样，或者vcrit的loader在vclazz的initialtingLoaderList中。
	 */
    ...
    Object* loader)
{
    ...
    LOGVV("--- Invoking loadClass(%s, %p)", dotName, loader);
    {
        ...
        JValue result;
		/*
		 * dvmCallMethod是dalvik或Java中调用函数的方法
		 * loadClass是java.lang.ClassLoader中的函数，用来加载函数
		 */
...
{
    return findClassNoInit(descriptor, NULL, NULL); // 重点
}
    ...
    const DexClassDef* pClassDef, Object* classLoader)
{
    ...
    if (pEncodedData != NULL) {
		// 解析类的一些信息，包括静态成员个数、非静态成员个数、实函数和虚函数的个数
    ...
    Method* meth)
{
	...
	/*
	 * dalvik中，函数信息由DexMethodId表示。
	 * 无非也就是函数名、参数类型、权限、所属类，函数的代码等信息
	 */
    ...
    if (pDexCode != NULL) {
		...
		// 如果是native函数，则先用dvmResolveNativeMethod顶着
    ...
    const DexField* pDexSField, StaticField* sfield)
{
	...
	/*
	 * dalvik中，静态成员由DexFieldId表示，DexFieldId也用于表示非静态成员。
	 * 它包括：
	 * 1.clazz:所属类
	 * 2.name:变量名
	 * 3.signature:类型信息，字符串，比如Ljava/lang/String;
	 * 4.accessFlags:访问权限
	 */
    sfield->clazz = clazz;
...
noverify:
    ...
    if (clazz->super != NULL && clazz->super->status != CLASS_INITIALIZED) {
        ...
        if (!dvmInitClass(clazz->super)) { // 如果基类还没有初始化，则先初始化基类
            assert(dvmCheckException(self));
            clazz->status = CLASS_ERROR;
     ...
     */
    initSFields(clazz); // 初始化目标类的静态成员变量

    /* 
	 * Execute any static initialization code.
	 * 调用目标类的<clinit>函数进行初始化
     */
    ...
    if (dvmCheckException(self)) {
        ...
        /* success! */
        dvmLockObject(self, (Object*) clazz);
        clazz->status = CLASS_INITIALIZED; // 到此，目标类就算初始化完成了
    ...
    const u2* insns)
{
	...
	/*
	 * 1.method->insns=native函数指针
	 * 2.method->nativeFunc=dvmCallJNIMethod
	 * 所以，dalvik并不是直接把指针设置到nativeFunc，而是把dvmCallJNIMethod设置到nativeFunc。
	 * 和前面讲Java函数调用一样，dalvik调用native函数前也需要准备栈帧
	 */
    if (insns != NULL) {
