    ...
    bool findVirtual, bool isHier, const char* name, const char* descriptor)
{
	...
	// 拆解descriptor字符串，得到返回值类型和参数个数，简单的字符串操作
    ...
    while (clazz != NULL) {
        ...
        size_t methodCount;
        size_t i;
		// 如果搜索虚函数，就从类的virtualMethods数组里找，否则从directMethods数组里找
        if (findVirtual) {
        ...
        for (i = 0; i < methodCount; i++) {
			...
			/*
			 * 比较Method的名字、参数类型、参数个数以及返回值是否一致。
			 * compareMethodHelper其实不难，但是由于dalvik在dex文件中存储Class信息和VM
			 * 标准略有不同，所以此函数内部需要读取dex文件里对应的信息，才能取到Method
			 * 的参数类型、返回值类型等。
			 */
...
/*
 * 这个函数的参数：
 * 1.clazz：代表目标Class的ClassObject
 * 2.methodName：函数名，比如add、main
 * 3.descriptor：函数signature，包含参数和返回值类型，比如(Ljava/lang/String;)V
 * 表示输入参数1个，类型是String，返回值类型为void
 */
