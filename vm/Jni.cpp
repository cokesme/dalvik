    ...
    const char* signature, void* fnPtr)
{
	...
	/*
	 * 参数methodName是java函数名，signature是该函数的签名"(参数信息)返回值类型"
	 * 当签名信息第一个字符是"!"，表示为fastJni模式
	 */
    bool fastJni = false;
    if (*signature == '!') {
        fastJni = true;
	...
	// 从ClassObject里根据函数信息找到对应的Method对象
    ...
    dvmUseJNIBridge(method, fnPtr); // 重要
...
void dvmUseJNIBridge(Method* method, void* func) {
    ...
    dvmSetNativeFunc(method, bridge, (const u2*) func); // 重要
}
...
void dvmCallJNIMethod(const u4* args, JValue* pResult, const Method* method, Thread* self) {
	...
	/*
	 * 不同平台的汇编代码实现的函数，大致功能还是解析参数并压栈，
	 * 然后跳转到insns指定的函数位置去执行。
	 */
...
{
    ScopedJniThreadState ts(env);
	// Java层传过来的可能是个WeakReference或者局部变量。但是我们要找到它实际指向的那个对象。
    ClassObject* clazz = (ClassObject*) dvmDecodeIndirectRef(ts.self(), jclazz);
...
jint JNI_CreateJavaVM(JavaVM** p_vm, JNIEnv** p_env, void* vm_args) {
	...
	/*
	 * gDvm全局变量，数据类型为DvmGlobals，该结构体是Dalvik的核心数据结构，
	 * 几乎所有的重要成员，控制参数（比如堆栈大小、状态、已经加载的类信息）
	 * 等都通过gDvm来管理，dalvik虚拟机大部分重要数据结构都由gDvm来存储
	 */
     ...
     */
	/*
	 * JavaVM在JNI语义中代表虚拟机本身
	 * Dalvik中，该虚拟机真实的数据类型就是JavaVMExt
	 */
     ...
     */
	/*
	 * JNIEnv在Jni语义中代表Jni环境
	 * Dalvik中，该环境的真实数据结构类型是JNIEnvExt
	 */
