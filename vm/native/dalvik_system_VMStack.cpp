...
static void Dalvik_dalvik_system_VMStack_getCallingClassLoader(const u4* args,
    JValue* pResult)
{
	/* dvmGetCaller2Class返回caller's caller所属的class，
	 * 这些信息来自于栈帧（往栈上偏移2次栈帧就可以了）
	 * caller's caller就是调用System.loadLibrary的类
	 */
    ClassObject* clazz =
    ...
    if (clazz == NULL)
        RETURN_PTR(NULL);
    RETURN_PTR(clazz->classLoader); // 返回这个类的ClassLoader
}
