    ...
    JValue* pResult)
{
    ...
    if (ldLibraryPathObj != NULL) {
		...
		/* 从已经加载的so里搜索android_update_LD_LIBRARY_PATH函数，此函数在
		 * bionic/linker/dlfcn.cpp里有定义，主要是处理ldLibraryPath里一些重复路径的情况
		 */
    ...
    char* reason = NULL;
    bool success = dvmLoadNativeCode(fileName, classLoader, &reason); // 真正的加载
    if (!success) {
