    ...
    JarFile** ppJarFile, bool isBootstrap)
{
     ...
     */
	// 首先尝试同目录下 是否有同名.odex文件。一些OEM厂商往往会预先生成odex文件
    fd = openAlternateSuffix(fileName, "odex", O_RDONLY, &cachedName);
    if (fd >= 0) {
        ALOGV("Using alternate file (odex) for %s ...", fileName);
		// 检查这些预置的odex文件是否过时。
...
tryArchive:
        ...
        if (entry != NULL) {
            ...
            ALOGV("dvmJarFileOpen: Checking cache for %s (%s)",
                fileName, cachedName);
			/*
			 * 尝试打开dalvik-cache下的dex文件，如果没有就创建它，并设置newFile=true
			 * 如果有，则要校验cache的dex文件是否需要更新。返回的fd就是代表cache的dex文件
			 */
            fd = dvmOpenCachedDexFile(fileName, cachedName,
                    dexGetZipEntryModTime(&archive, entry),
                    dexGetZipEntryCrc32(&archive, entry),
            ...
            if (newFile) {
                ...
                if (result) {
					/*
					 * 启动dexopt进程对dex文件进行odex优化。最终的odex文件存储在fd
					 * 也就是dalvik-cache下那个.dex文件。虽然后缀是.dex，其内容却是odex
					 */
     ...
     */
	// 打开odex文件，并做一些解析工作。此函数干了不少事情，但此处略
    ...
    *ppJarFile = (JarFile*) calloc(1, sizeof(JarFile));
    (*ppJarFile)->archive = archive; // 代表xxx.jar
    (*ppJarFile)->cacheFileName = cachedName; // odex文件名，位于/data/dalvik-cache下
    (*ppJarFile)->pDvmDex = pDvmDex; // 代表解析得到的odex文件信息
