...
{
	/*
	 * Thread.h中Thread是Dalvik中一个比较复杂的数据结构，里面包含了很多
	 * 与Java代码执行过程相关的信息，比如栈、字节码执行情况（由InterpSaveState，
	 * 内含当前字节码执行的位置，也就是PC等）
	 */
    Thread* thread;

    /* allocate a TLS slot */
	// 由于Thread包含了重要信息，所以当线程退出时，需要通过threadExitCheck进行资源释放的检查
 ...
 */
static Thread* allocThread(int interpStackSize) // dalvik默认线程栈大小是16k
{
    Thread* thread;
    u1* stackBottom;
    ...
    thread->interpStackSize = interpStackSize; // 栈大小
	// 整个栈的起始位置为stackBottom。stackBottom和interpStackEnd还有一个768字节的保护区域。如果栈内容到这块区域就认为出错了。
    thread->interpStackStart = stackBottom + interpStackSize; // 栈顶，位于内存高地址
    thread->interpStackEnd = stackBottom + STACK_OVERFLOW_RESERVE; // 栈底，位于内存低低地址
