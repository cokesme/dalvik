...
# define FINISH(_offset) {                                                  \
        ADJUST_PC(_offset); /*调整PC的位置*/                                \
        inst = FETCH(0); /*获取当前PC指令的前个字节，并保存到inst变量中，inst类型为u2，即dex中指令的前两个字节为操作码*/                                                                      \
...
#define GOTO_invoke(_target, _methodCallRange) /*(invokeDirect, false)*/    \
    do {                                                                    \
        methodCallRange = _methodCallRange; /*methodCallRange在dvmInterpret函数中定义，是局部变量*/ \
        goto _target;                                                       \
    } while(false)
...
void dvmInterpretPortable(Thread* self)
{
    ...
    pc = self->interpSave.pc; // 就是method->insns，也就是这个方法的第一个字节码指令。
...
/* File: c/OP_NOP.cpp */
/* 
 * File表示这段代码从哪个文件里过来的，目录dalvik/vm/mterp/c
 * HANDLE_OPCODE:定义goto label
 * 注意，后面一大段代码其实都位于dvmInterpretPortable这个函数里
 * OP_END:表示这段goto label结束，无实际意义，该宏为空定义
 */
...
HANDLE_OPCODE(OP_MOVE /*vA, vB*/)
    ...
    FINISH(1); // 向下移动PC，以指向接下来的指令，根据不同的操作符，偏移值也不同，OP_MOVE是1,还有2,3的
OP_END
...
HANDLE_OPCODE(OP_NEW_INSTANCE /*vAA, class@BBBB*/)
    {
		...
		/*
		 * dvmDexGetResolvedClass:看看目标类(要new的类)是不是已经解析过了。
		 * methodClassDex:代表当前所执行方法对应的dex文件。
		 * ref:代表目标类的索引。
		 */
        clazz = dvmDexGetResolvedClass(methodClassDex, ref);
		// 如果目标类没有解析过
        if (clazz == NULL) {
			// dvmResolveClass加载目标类，curMethod->clazz指向当前所执行方法所在类。
            clazz = dvmResolveClass(curMethod->clazz, ref, false);
            if (clazz == NULL)
                GOTO_exceptionThrown();
        }
